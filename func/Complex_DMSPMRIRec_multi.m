function  map = Complex_DMSPMRIRec_multi(gt, Im, partialdata, mask, params, net1, net2 ,net3)
...
for iter = 1:params.num_iter
    ...
    for iiii = 1:repeat_num
        ...
        % compute prior gradient 1
        input = real(map_all(:,:,[3,2,1])); % Switch channels for caffe
        noise = randn(size(input)) * params.sigma_net3;
        rec = Processing_Im_w(single(input+noise)/255, net3, params.gpu, params.out_idx);
        rec = double(rec)*255;
        prior_err = input - rec;
        prior_err1 = mean(prior_err,3);
    ...
    if any(strcmp('gt',fieldnames(params)))
        ...
        disp(['PSNR is: ' num2str(psnr) ',''HFEN is: ' num2str(hfen) ',', 'iteration finished in ' num2str(toc()) ' seconds']);
    end
